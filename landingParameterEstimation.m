function landingParameterEstimation(track, runDemo)
...
if runDemo
    ...
    iodata = [track.state(:,6)./track.state(:,3), track.state(:,6), track.state(:,3)]; % [r Vgy y]
end
...
VgyError     = evalRequirement(ref_sig, VgySignal.Values, Exp.OutputData(2).Values);
yError       = evalRequirement(ref_sig, ySignal.Values, Exp.OutputData(3).Values);

%%
% Return the errors to the optimization solver.
%
vals.F = [rError(:); VgyError(:); yError(:)];
end
